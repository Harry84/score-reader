...
**Things not yet verified live:** none of the four `awaiting_*` pauses (`awaiting_player_match`, `awaiting_team_assignment`, and the two newest - `awaiting_roster_size`, `awaiting_missing_field`) have been triggered through the actual bot yet - every live Discord test so far happened to hit the unambiguous, fully-populated 5-a-side path. All four have full automated test coverage in `tests/ingestion`/`tests/api`/`tests/bot`, but the live Discord round-trip (post a screenshot that's missing a player, or with a null stat, or an unrecognized name, and answer the bot's question) hasn't been watched happen. Worth doing before calling Phase 3 fully closed out.
...
**Done: Phases 0, 1, 2, 3.** All on branch `feature/containerise`, one commit per vertical slice, pushed to `origin/feature/containerise`. 89 tests passing across `tests/db`, `tests/ingestion`, `tests/api`, `tests/stats`, `tests/teams`, `tests/bot`, plus the pre-existing suite (see note below). Real dev Postgres (`docker compose up postgres`, port 5433) has the real migrated historical data (5 seasons, 17 matches) and is on the latest migration (`0011_pending_matches_status_validation.sql`).
